...
framebuffer* createFrameBuffer(height, width){
    temp* framebuffer = malloc(sizeof(framebuffer));
...
void drawline(framebuffer* fb, triangle tri){
    //one to two
    float dx1 = tri.one.x - tri.two.x;
    float dy1 = tri.one.y - tri.two.y;
    float len1 = sqrt(dx1 * dx1+ dy1 * dy1;
    float angle1 = atan2(dy1, dx1);
    for(float i = 0; < len1; i++){
        colorPixel(fb,
        round(tri.one.x + cos(angle1) * i),
        round(tri.one.y + sin(angle1) * i), 
        255, 255, 255);
    }
    //two to three
    ...
    for(float i = 0; < len1; i++){
        colorPixel(fb,
        round(tri.two.x + cos(angle2) * i),
        round(tri.two.y + sin(angle2) * i), 
        255, 255, 255);
    }
    //thee to one
    float dx3 = tri.three.x - tri.two.x;
    float dy3 = tri.three.y - tri.two.y;
...
void drawLine(SDL_Renderer* renderer,vec3* a, vec3* b){
    float dx = b->x - a->x;
    float dy = b->y - a->y;
